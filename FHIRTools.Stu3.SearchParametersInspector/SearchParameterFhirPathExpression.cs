...
namespace FHIRTools.Stu3.SearchParametersInspector
{
  public class SearchParameterFhirPathExpression
  {
    ...
    private string FilePath = @"C:\temp\STU3FhirSpecExampleSearchParametersExpCheck.txt";

    public void Process()
    {
      StringBuilder sb = new StringBuilder();
      sb.AppendLine($"{"Resource".PadRight(27, ' ')}, {"Expression".PadRight(30, ' ')}, {"Error".PadRight(40, ' ')}");
      sb.AppendLine($"-".PadRight(100, '-'));

      FhirSpecExamplesTool = new FhirSpecExamplesTool();
      FHIRTools.Stu3.Common.SearchParameterTools SearchParameterTools = new Common.SearchParameterTools();
      var SearchParamneterDefList = SearchParameterTools.GetSearchParameterDefinitionList();

      FHIRTools.Stu3.Common.FhirPathTools.FhirPathProcessor FhirPathProcessor = new Common.FhirPathTools.FhirPathProcessor();
            ...
            {              
              ...
              foreach(var Res in ExampleList)
              {
                ...
                if (!CompiledExpressionOutrCome.Ok)
                {
                  ...
                  {
                    sb.AppendLine($"{Res.ResourceType.GetLiteral().PadRight(27, ' ')}, {CompiledExpressionOutrCome.ParseExpressionOutCome.ExpressionString.PadRight(30, ' ')}");
                  }
                  else
                  {
                    sb.AppendLine($"{Res.ResourceType.GetLiteral().PadRight(27, ' ')}, {CompiledExpressionOutrCome.ParseExpressionOutCome.ExpressionString.PadRight(30, ' ')}, {CompiledExpressionOutrCome.ErrorMessage.PadRight(40, ' ')}");
                  }                  
